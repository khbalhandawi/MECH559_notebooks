...
def get_inputs(V:List[variableData], names:List[str], sp_index:int):
  ...
  return [vi for vi in V if vi.name in names and vi.sp_index==sp_index and vi.coupling_type not in [COUPLING_TYPE.FEEDFORWARD]]

def get_outputs(V:List[variableData], names:List[str], sp_index:int):
  """This function returns a list of variable objects based on name and subproblem index"""
  return [vi for vi in V if vi.name in names and vi.sp_index==sp_index and vi.coupling_type in [COUPLING_TYPE.FEEDFORWARD]]
...
def SBJ_A1(x):
  ...
  # ["D","Wt","theta","ESF","L"] # these are your responses
  y = [inputs["D"],Wt,inputs["theta"],inputs["ESF"],inputs["L"]]
...
def SBJ_opt1(x, y):
  ...
  outputs = {}
  for outputs_name,value in zip(["D","Wt","theta","ESF","L"],y): # these are your responses
...
def SBJ():
  ...
  y1 = get_outputs(V,["D","Wt","theta","ESF","L"],sp_index=1) # outputs
  V1 = copy.deepcopy(v1)
  Y1 = copy.deepcopy(y1)
